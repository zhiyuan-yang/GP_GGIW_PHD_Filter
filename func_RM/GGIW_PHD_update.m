function [w_k,a_k,b_k,m_k,P_k,nu_k,V_k,J_k] = ...
    GGIW_PHD_update(w_kk1,a_kk1,b_kk1,m_kk1,P_kk1,nu_kk1,V_kk1,J_kk1,Zk)
...
% Clutter rate
beta_FA = 20 / 900; 
%model.obs.lambda_c * model.obs.pdf_c;
    ...
    for w = 1:W(p)
        ...
        for j = 1:J_kk1
            ...
            if absW < 2
                ...
                % kinematic update
                zbar = mean(pwZ_k,2);
                innov = zbar-kron(H_k,Id)*m_kk1(:,j);   %(32e)
                S = H_k*P_kk1(:,:,j)*H_k'+ 1/absW; %(32c)
                K = P_kk1(:,:,j)*H_k'* 1/S;   %(32d)
                m_k(:,J_kk1*l+j) = m_kk1(:,j) + kron(K,Id) * innov;  %(31a)
                
                % covariance update
                P_k(:,:,J_kk1*l+j) = P_kk1(:,:,j) - K*S*K';  %(31b)
                P_k(:,:,J_kk1*l+j) = 0.5*(P_k(:,:,J_kk1*l+j)+P_k(:,:,J_kk1*l+j)');

                % extend update
                nu_k(1,J_kk1*l+j) = nu_kk1(1,j) + absW;  %(31c)
                N = inv(S)*innov*innov';                        %(32f)
                bar_Z = (absW - 1) * cov(pwZ_k');
                V_k(:,:,J_kk1*l+j) = V_kk1(:,:,j)+ N + bar_Z;   %(31d)
                V_k(:,:,J_kk1*l+j) = 0.5*(V_k(:,:,J_kk1*l+j)+V_k(:,:,J_kk1*l+j)');
